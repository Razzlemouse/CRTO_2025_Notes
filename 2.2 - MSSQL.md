...
- If none found, try with portscan
```powershell
...
- If you are an admin, achieve RCE with CLR:
...
- Add it with the existing item, set properties to embedded. Then execute:
```powershell
...
```
- Link to the SMB BEACON after loading the DLL
...
#When finished, disable for OPSEC
beacon> sql-disablexp lon-db-1
#Do the same with lon-db-2
```
